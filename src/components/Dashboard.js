...
const Dashboard = () => {
  ...
  const callOpenAI = async (prompt) => {
    const apiKey =
      "k-proj-GGoMje_MujJX4Ugdf2u0CFuVeufgVYujdpPXiYe1wOcBgEi_UVkcLXsk68NjwgsZ4W0KJ-YecKT3BlbkFJsBY5yxR_2fTQMUXPfJ5njr6mdd8Znpkm5pIk3QLXuxq_JRqQoJJjj3W03hxHbH0QnaVTj-9a0A";
    if (!apiKey) {
      console.error(
        "OpenAI API key not found. Make sure REACT_APP_OPENAI_API_KEY is set."
      );
  ...
  const fetchDataAndGenerateDashboard = useCallback(
    async (teamId, userQuery = "", currentFilter = "all") => {
      console.log(
        `fetchDataAndGenerateDashboard called with teamId: ${teamId}, query: ${userQuery}, filter: ${currentFilter}`
      );
      if (!teamId) return;
      console.log(
        `Fetching data for team: ${teamId}, Filter: ${currentFilter}`
      );
      setIsLoading(true);
      ...
      try {
        ...
        const { data: responseData, error: responsesError } =
          await supabase.rpc("get_survey_responses_for_team", {
        ...
        console.log(
          ...
          } responses with filter: ${currentFilter}`
        );

        if (!responseData || responseData.length === 0) {
          // Check if responseData is null/empty
...
`;
          ...
          try {
            ...
            if (Array.isArray(suggestionsResult)) {
              ...
              console.log(
                ...
                suggestionsResult.slice(0, 3)
              );
            } else {
              console.warn(
                "OpenAI suggestion response was not an array:",
                suggestionsResultString
              );
              setDynamicPromptSuggestions([]); // Fallback to empty
            }
          } catch (suggestionParseError) {
            console.error(
              "Failed to parse JSON suggestions from OpenAI:",
              suggestionParseError
            );
            console.error(
              "Raw OpenAI Suggestion Response:",
              suggestionsResultString
            );
          ...
          console.error(
            "Error generating prompt suggestions:",
            suggestionError
          );
...
${
  userQuery
    ? `The user has specifically asked: '${userQuery}'. Please prioritize generating insights and visualizations directly related to this request.`
    : "Generate a general overview dashboard based on the data."
}
...
`;
        ...
        const validDashboardCards = (openAIResult.dashboard_cards || []).filter(
          (card) =>
            card.data &&
            card.data.datasets &&
            card.data.datasets.length > 0 &&
            card.data.datasets[0].data
        );
    ...
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const teamIds = teamMemberships
          ? teamMemberships.map((m) => m.team_id)
          : [];
        ...
        if (teams.length > 0) {
          ...
          setCurrentTeam((prevTeam) => {
            ...
            console.log(
              "Dashboard: Current team already exists, not setting initial team."
            );
  ...
  useEffect(() => {
    if (currentTeam?.id) {
      console.log(
        `Team or Filter changed. Fetching data for team ${currentTeam.id} with filter ${surveyFilter}`
      );
  ...
  const handleExportToPDF = () => {
    ...
    const opt = {
      margin: 10,
      filename: `${currentTeam.replace(/\s+/g, "_")}_Dashboard_${
  ...
  const handleTeamChange = (event) => {
    ...
    if (selectedTeam) {
      ...
      console.error(
        "Dashboard: Selected team not found in options",
        selectedTeamId
      );
    }
  };
  ...
  return (
    <div className="dashboard-container">
      <div className="dashboard-title-section">
        <h1>Dashboard</h1>
        <p>
          Your own customizable insights dashboard for monitoring performance
      ...
      <div className="stats-overview">
        ...
        {!isLoading && statCards.length === 0 && (
          <p className="no-data-message">
            No stats to display for the selected filter.
          </p>
        )}
        {statCards.map((stat) => (
      ...
      <div className="dashboard-cards">
        {isLoading && dashboardCards.length === 0 && (
          <p className="loading-message">Loading charts...</p>
        )}
        {!isLoading && dashboardCards.length === 0 && (
          <p className="no-data-message">
            No charts to display for the selected filter.
          </p>
        )}
          ...
          <div key={card.id} className="dashboard-card">
            ...
            <div className="chart-container">
              <ResponsiveContainer width="100%" height="100%">
                {card.type === "line" && (
                  <AreaChart
                    data={card.data.datasets[0].data.map((value, i) => ({
                      name: card.data.labels[i],
                      value,
                    }))}
                  >
                    <defs>
                      <linearGradient
                        id={`color-${card.id || "default"}`}
                        x1="0"
                        y1="0"
                        x2="0"
                        y2="1"
      ...
      <div className={`ai-side-panel ${aiSidePanelExpanded ? "expanded" : ""}`}>
        ...
        <div className="prompt-suggestions">
          ...
          <div className="suggestion-buttons-container">
            {isLoading && dynamicPromptSuggestions.length === 0 && (
